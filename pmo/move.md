...
- сделал скриншот реализованной части ТЗ и приложил его в комменты,
...
## Ревью
Задача каждого разработчика пройти ревью у своих коллег. Чтобы инициировать ревью по задаче, необходимо изменить статус родительской задачи на Ревью. И сообщить, в произвольной форме, ревьюеру о наличии у него задачи.

Ответственный за ревью ежедневно разбирает пришедшие к нему задачи. По возможности, делать это как можно быстрее, чтобы не задерживать задачу своих коллег.

Если задача не прошла ревью, то проверяющий:
- фиксирует все замечания в Гитлабе,
- пишет короткий комментарий к задаче в КликАпе (Доработки по комментариям) и меняет её статус на Разработка.

Если прошла, то в произвольном порядке, сообщает об этом ответственному разработчику и тот, в свою очередь, двигает задачу в статус Тестирование.
...
В тестирование берутся только родительские задачи,ответственный тестировщик заходит в задачу и назначает на себя соответствующую подзадачу и двигает ее в статус Тестирование.
...
Дефекты заводятся в комментарии к родительской задаче в виде нумерованного списка.

По окончании тестирования, в случае наличия критических, блокирующих или большого количества прочих дефектов возвращает задачу в разработку, путем установки у родительской задачи статуса Разработка.
...
- Relationship = Родительская задача.
